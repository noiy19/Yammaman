...
export function ProductGrid({ block }: { block: BlockOf<'productGrid'> }) {
  const { heading, intro, source, limit, columns = 3 } = block.props;
    ...
    <Section labelledBy={heading ? headingId : undefined}>
      <Container>
        ...
        <div className="mt-10">
          ...
          {data && data.length > 0 ? (
            <ul
              className={`grid list-none grid-cols-1 gap-8 p-0 ${COLUMNS[columns] ?? COLUMNS[3]}`}
            >
              {data.map((product) => (
                <li key={product.productId}>
                  ...
                  />
                  <h3 className="text-ink mt-4 text-base font-medium">
                    {product.name}
                  </h3>
                  <p className="text-ink-secondary mt-1 text-sm">
                    {product.priceLabel}
                  </p>
                  <p className="text-ink-dimmed mt-1 text-xs tracking-wide uppercase">
